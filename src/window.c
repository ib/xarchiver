...
void xa_show_archive_comment (GtkMenuItem *menuitem,gpointer user_data)
{
	...
	if (archive[idx]->comment)
	{
		...
		{
			gchar *utf8 = g_locale_to_utf8(archive[idx]->comment->str, -1, NULL, NULL, NULL);
		...
		gtk_text_buffer_insert_with_tags_by_name(textbuffer, &iter, archive[idx]->comment->str, -1, "font", NULL);
	}
