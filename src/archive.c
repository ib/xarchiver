...
void xa_spawn_async_process (XArchive *archive, gchar *command)
{
		...
		&error))
	{
		...
		g_strfreev (argv);
		archive->child_pid = 0;
		xa_set_button_state (1,1,1,1,archive->can_add,archive->can_extract,archive->has_sfx,archive->can_test,archive->has_properties,archive->has_passwd,1);
		return;
	}
	g_strfreev (argv);
...
gboolean xa_run_command (XArchive *archive,GSList *commands)
{
	...
	if (xa_main_window)
		xa_set_button_state (1,1,1,1,archive->can_add,archive->can_extract,archive->has_sfx,archive->can_test,archive->has_properties,1,1);
